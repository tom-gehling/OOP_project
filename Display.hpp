...
class Display {
    ...
    void create() { // creates window
        ...
        wtimeout(display_win, 1000); // 'refresh rate' of window set to 1s
    ...
    void addText(int y, int x, char* str) { // adds a string text at a specified coordinate
        mvwprintw(display_win, y, x, str);
    }

    chtype getInput() { // gets input from user
    ...
    void delete_window() { // deletes window (cannot be called again after)
        endwin();
        delwin(display_win);
    }
